...
/**
 * Viewarticle Función para desplegar el contenido del artículo
 */
function viewarticle($artid, $page) {
    global $prefix, $db, $module_name, $pagetitle,$admin,$currentlang,$multilingual,$subsmetas,$sloganaux;
  ...
  $row2 = $db->sql_fetchrow($db->sql_query("SELECT secname from ".$prefix."_sections_titles
   where secid='$secid' and language='$currentlang'"));
	...
	$descrip2=str_replace("imagen-abre-","<a href=\"cancun-photo-",$descrip); 
    $descrip3=str_replace("-imagen-cierra",".html\" onclick=\"NewWindow(this.href,'mywin','802','380','no','center');return false\">",$descrip2); 
...
function viewarticle_old($artid, $page) {
    global $prefix, $db, $module_name, $pagetitle,$admin,$currentlang,$multilingual,$subsmetas,$sloganaux;
...
function PrintSecPage($artid) {
	...
	$descrip2=str_replace("imagen-abre-","<a href=\"modules.php?name=Photos&pid=",$content); 
    $descrip3=str_replace("-imagen-cierra",">",$descrip2);
